...
LOOM 0.15.70 makes release identity self-consistent: the browser derives its running release from canonical boot/config/script evidence and only announces a strictly newer semantic version. Same-version manifest fingerprints are deployment/integrity metadata, not a second release.
...
This release also removes the duplicate Admin → Users navigation entry, hardens visible-name precedence so historical internal handles cannot unexpectedly replace a newer display name, and retains the generic project leaderboard/import infrastructure without bundling any concrete project implementation. Release ZIPs remain clean-instance packages: `instance/**` is never shipped.


## 0.15.59 — generic project leaderboard + smart project overwrite imports

LOOM supports project-specific telemetry and leaderboards without turning application-specific UI into core modules. A generic project-scoped leaderboard state API provides identity-aware persistence while concrete leaderboard UI and scoring logic remain inside the owning Instance Project. Project apps can report native lifetime metrics and explicit active state so durable totals survive sessions and playtime measures application activity rather than generic iframe presence.
...
- A one-word project name remains one word. `Atlas`, for example, renders as `ATLAS` with no invented second word.
