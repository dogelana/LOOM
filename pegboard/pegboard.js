...
const project=(qs.get('project')||'green-beans').replace(/[^a-z0-9_-]/g,'');
const apiBase='../api',fallback=`../projects/${project}/registry.fallback.json`;
...
document.getElementById('themeSelect').onchange=e=>{document.body.classList.remove('theme-green-beans','theme-loom-dark');document.body.classList.add(`theme-${e.target.value}`);localStorage.setItem('loom:pegboard-theme',e.target.value)};
const savedTheme=localStorage.getItem('loom:pegboard-theme')||'green-beans';document.getElementById('themeSelect').value=savedTheme;document.body.classList.remove('theme-green-beans','theme-loom-dark');document.body.classList.add(`theme-${savedTheme}`);
