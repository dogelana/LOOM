...
export async function createModule(ctx){
  ...
  function render(){
    ...
    for(const d of eligible()){
      const st=settingsFor(d);if(!st.captured)continue;const item=document.createElement('button');item.type='button';item.className='loom-orb-item';item.dataset.target=d.action.id;item.title=st.label;
  ...
  return{async mount(){},async activate(){render();await ctx.log('orb-dock.ready',{captured:eligible().filter(d=>settingsFor(d).captured).map(d=>d.action.id)});return()=>{}},async deactivate(){observer?.disconnect();for(const [id,loc] of moved){const n=targetNode(id);if(!n)continue;n.classList.remove('loom-orb-captured');n.hidden=false;if(loc.parent?.isConnected)loc.parent.insertBefore(n,loc.next?.isConnected?loc.next:null);else ctx.mountRoot.appendChild(n)}overlay?.remove();root?.remove()},async unmount(){observer?.disconnect();overlay?.remove();root?.remove()}};
}