...
## v0.15.34 architecture note — content lane vs page lane

Project pages now distinguish ordinary readable content from full-canvas surfaces. The default root flow stays capped at 1320px. A root-mounted module may explicitly declare `presentation.layout.widthScope = "page"` to use the shell's full usable inline width without widening unrelated modules. HTML Framer is the first canonical consumer of this contract. Its 50–100% width setting is measured inside the page lane and defaults to 100%.

## v0.15.33 architecture note — products are instances, not release payload

LOOM core is project-agnostic. Production release archives ship the engine, generic baseline template, reusable/core modules, and Instance Project protocol, but no concrete product project. Green Beans is now expected to travel as a portable project bundle and import into `instance/` as an independently owned Instance Project. This keeps LOOM upgrades from carrying, overwriting, or implicitly recreating product code.

Developer tools must receive project context explicitly. No LOOM core surface may silently substitute Green Beans when a project parameter is absent.
