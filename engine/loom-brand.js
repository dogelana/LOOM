// @loom-file release=0.15.74 revision=75 policy=package-priority
(()=>{
  const HARD_RELEASE='0.15.74';
  ...
  function injectStyle(){if(document.getElementById('loom-brand-core-style'))return;const s=document.createElement('style');s.id='loom-brand-core-style';s.textContent=`
    ...
    .loom-shell-chrome-header{width:100%;display:flex;align-items:center;justify-content:space-between;gap:16px;padding:12px max(16px,calc((100vw - 1240px)/2));border-bottom:1px solid rgba(194,214,199,.78);background:linear-gradient(180deg,rgba(255,255,255,.94),rgba(248,252,249,.88));backdrop-filter:blur(22px) saturate(130%);-webkit-backdrop-filter:blur(22px) saturate(130%);position:relative;z-index:500;box-sizing:border-box;box-shadow:0 10px 34px rgba(21,63,32,.06),inset 0 1px rgba(255,255,255,.9)}
    .loom-shell-chrome-brand{display:flex;align-items:center;gap:11px;min-width:0;color:inherit;text-decoration:none;border-radius:18px;transition:transform .15s ease,box-shadow .15s ease}.loom-shell-chrome-brand:hover{transform:translateY(-1px)}.loom-shell-chrome-brand:focus-visible{outline:3px solid rgba(50,174,96,.20);outline-offset:3px}.loom-shell-chrome-cube{width:58px;height:58px;flex:0 0 58px;display:grid;place-items:center;padding:11px;overflow:hidden;border:1px solid rgba(201,218,205,.72);border-radius:17px;background:linear-gradient(145deg,rgba(255,255,255,.92),rgba(243,249,245,.88));box-shadow:0 8px 24px rgba(22,68,35,.07),inset 0 1px #fff}.loom-shell-chrome-text{min-width:0}.loom-shell-chrome-title{font:950 13px/1.05 Inter,system-ui;color:#183522;letter-spacing:-.018em}.loom-shell-chrome-sub{margin-top:4px;font:800 9px/1.1 Inter,system-ui;color:#77867c;letter-spacing:.08em;text-transform:uppercase}
    .loom-shell-chrome-links{display:flex;align-items:center;gap:8px;flex-wrap:wrap;justify-content:flex-end}.loom-shell-chrome-links a,.loom-shell-chrome-links button{box-sizing:border-box;min-height:42px;display:inline-flex;align-items:center;justify-content:center;gap:7px;padding:8px 11px;border:1px solid #d6e4d9;border-radius:12px;background:linear-gradient(180deg,#fff,#f6faf7);color:#345c40;text-decoration:none;font:900 10px/1 Inter,system-ui;cursor:pointer;box-shadow:0 4px 14px rgba(22,68,35,.045);transition:transform .15s ease,border-color .15s ease,box-shadow .15s ease,background .15s ease}.loom-shell-chrome-links>a .loom-global-nav-emoji{width:16px;height:16px;display:inline-grid;place-items:center;flex:0 0 16px;font-size:16px;line-height:1;font-family:"Apple Color Emoji","Segoe UI Emoji","Noto Color Emoji",sans-serif}.loom-shell-chrome-links a:hover,.loom-shell-chrome-links button:hover{transform:translateY(-1px);border-color:#b3d2bb;background:#fff;box-shadow:0 9px 22px rgba(22,68,35,.09)}.loom-shell-chrome-links a:focus-visible,.loom-shell-chrome-links button:focus-visible{outline:3px solid rgba(50,174,96,.20);outline-offset:2px;border-color:#8fc7a0}
    .loom-shell-chrome-footer{width:min(1240px,calc(100% - 28px));margin:34px auto 20px;display:flex;justify-content:center;box-sizing:border-box;color:#526359}
    .loom-shell-footer-pill{width:max-content;max-width:100%;padding:15px 20px;border:1px solid rgba(205,221,209,.88);border-radius:22px;background:linear-gradient(145deg,rgba(255,255,255,.88),rgba(245,250,247,.84));backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px);box-shadow:0 18px 52px rgba(20,62,31,.075),inset 0 1px #fff;display:flex;flex-direction:column;align-items:center;gap:9px}
    ...
    @media(max-width:680px){.loom-shell-chrome-header{align-items:flex-start;flex-wrap:wrap;padding:10px}.loom-shell-chrome-brand{width:100%}.loom-shell-chrome-sub{display:none}.loom-shell-chrome-links{width:100%;justify-content:flex-start;flex-wrap:nowrap;overflow-x:auto;padding:2px 1px 5px;scrollbar-width:none}.loom-shell-chrome-links::-webkit-scrollbar{display:none}.loom-shell-chrome-links a,.loom-shell-chrome-links button{min-height:38px;font-size:9px;padding:8px 9px;flex:0 0 auto}.loom-shell-chrome-footer{width:calc(100% - 18px)}.loom-shell-footer-pill{max-width:100%}}
  ...
  async function mountShellHeader(host,opts={}){
    ...
    let homeHref=String(opts.homeHref||'');if(!homeHref){try{const apiUrl=new URL(String(opts.apiBase||'api').replace(/\/?$/,'/'),document.baseURI||location.href);homeHref=new URL('../home/',apiUrl).href}catch{homeHref='../home/'}}
    host.innerHTML=`<a class="loom-shell-chrome-brand" href="${homeHref.replace(/["<>]/g,'')}" title="Open LOOM Home" aria-label="Open LOOM Home"><div class="loom-shell-chrome-cube"></div><div class="loom-shell-chrome-text"><div class="loom-shell-chrome-title">LOOM</div><div class="loom-shell-chrome-sub">${String(opts.pageTitle||'Application Engine').replace(/[<>]/g,'')}</div></div></a><nav class="loom-shell-chrome-links"></nav>`;
    ...
    for(const link of opts.links||[]){
      ...
      const a=document.createElement('a');a.href=link.href;a.className='loom-global-control loom-shell-static-link';a.dataset.loomGlobalControl='shell-link';
      const label=String(link.label||'').trim(),parts=label.split(/\s+/),first=parts[0]||'',rest=parts.slice(1).join(' '),hasIcon=!!rest&&/[^\x00-\x7F]/.test(first);
      if(hasIcon){const icon=document.createElement('span');icon.className='loom-global-nav-emoji';icon.setAttribute('aria-hidden','true');icon.textContent=first;const text=document.createElement('span');text.textContent=rest;a.append(icon,text)}else a.textContent=label;
      if(link.target)a.target=link.target;nav.appendChild(a);
    }
    return stop;
  }
