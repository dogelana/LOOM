...
The internal slug is routing/storage identity only. It must never be used as visible fallback brand text when a human project name is available. `lint-away` therefore renders from `Lint Away`, and automatic wordmark balancing yields `LINT` / `AWAY`.
