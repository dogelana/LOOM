...
LOOM production releases no longer ship Green Beans, or any other concrete product, as an active release-managed project or project template. Product projects belong under the Instance Project protocol and move between installations with project export/import bundles. The historical Green Beans scratch-reset section below documents older releases only; it is not a current bootstrap behavior and must not recreate Green Beans on a fresh install.
## Naming
