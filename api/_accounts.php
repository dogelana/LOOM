<?php
// @loom-file release=0.15.64 revision=20 policy=package-priority
...
function loom_account_public_status(string $clientId,string $project=''): array {
  loom_reconcile_admin_identity($clientId);if(function_exists('loom_access_reconcile_system_owner_identity'))loom_access_reconcile_system_owner_identity($clientId);$auth=loom_auth_user();$linked=loom_account_user_for_client($clientId);$u=$auth?:$linked;$storedPrivilege=(string)($u['privilege']??'User');$effectiveAdmin=loom_client_is_admin($clientId);$isOwner=function_exists('loom_access_is_system_owner')?loom_access_is_system_owner($clientId):false;$source='user';
  if($isOwner)$source=loom_access_system_owner_user_id()!==''?'system-owner-account':'system-owner-bootstrap-browser';elseif($effectiveAdmin){if($auth&&strcasecmp((string)($auth['privilege']??'User'),'Admin')===0)$source='permanent-admin-account';elseif(loom_admin_cookie_valid())$source='bootstrap-admin-recovery';else $source='linked-admin-state';}
  ...
  $visible=function_exists('loom_guest_profile_visible_name_for_client')?loom_guest_profile_visible_name_for_client($clientId):'';$globalDisplay=is_array($global)&&function_exists('loom_global_profile_display_name')?loom_global_profile_display_name($global):'';$ownerState=function_exists('loom_access_system_owner_state')?(loom_access_system_owner_state()?:[]):[];return ['authenticated'=>(bool)$auth,'registered'=>(bool)$u,'userId'=>$u['user_id']??$u['userId']??null,'username'=>$identity['effectiveUsername']??($globalDisplay?:($visible?:($global['username']??null))),'projectUsername'=>$identity['effectiveUsername']??null,'globalDisplayName'=>$globalDisplay?:null,'globalUsername'=>$visible?:($global['username']??null),'internalProfileHandle'=>$global['username']??null,'accountHandle'=>$u['username']??null,'email'=>$u['email']??null,'privilege'=>$effectiveAdmin?'Admin':'User','accountPrivilege'=>$storedPrivilege,'effectivePrivilege'=>$effectiveAdmin?'Admin':'User','isSystemOwner'=>$isOwner,'authorityRole'=>$isOwner?'System Owner':($effectiveAdmin?'LOOM Admin':'User'),'systemOwnerAccountBound'=>!empty($ownerState['userId']),'systemOwnerUserId'=>$ownerState['userId']??null,'systemOwnerClientId'=>$ownerState['clientId']??null,'systemOwnerBootstrapMethod'=>$ownerState['bootstrapMethod']??null,'authorizationSource'=>$source,'privilegeMismatch'=>strcasecmp($storedPrivilege,$effectiveAdmin?'Admin':'User')!==0,'storageMode'=>loom_db_ready()?'database':'durable-local'];
}
