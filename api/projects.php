...
foreach($slugs as $slug){
  try{
    ...
    if(!loom_request_is_admin()&&!loom_access_can_view_project($requestClientId,$slug))continue;
    ...
    $visibility=loom_access_project_visibility($slug);$projects[]=['slug'=>$slug,'name'=>$data['name']??humanize_project_slug($slug),'tagline'=>$data['tagline']??'','description'=>$data['description']??'','bio'=>$bio,'bio_custom'=>$rawBio,'bio_is_fallback'=>$rawBio==='','brand_primary_color'=>$colors['primary'],'brand_accent_color'=>$colors['accent'],'social_color'=>$socialColor,'wordmark'=>loom_project_brand_identity($slug),'theme'=>$data['theme']??'default','version'=>$data['version']??'0.0.0','source'=>loom_project_source($slug),'branding'=>loom_home_project_branding($slug,$dir,$data),'app_url'=>loom_access_project_entry_url($slug,$requestClientId),'canonical_app_url'=>loom_project_app_url($slug),'domain_landing'=>loom_project_is_domain_landing($slug),'visibility'=>$visibility,'is_private'=>$visibility==='private','can_manage_visibility'=>$requestClientId!==''&&loom_access_is_system_owner($requestClientId),'project_role'=>$projectRole,'project_capabilities'=>$projectCaps,'can_project_admin'=>(bool)$canProjectAdmin,'pegboard_url'=>"pegboard/?project=$slug&v=".rawurlencode($release),'registry_url'=>"registry/?project=$slug&v=".rawurlencode($release)];
  }catch(Throwable $e){
