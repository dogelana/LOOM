...
try{
  if($action==='status'){
    $legacy=safe_token((string)($body['legacyClientId']??''));$profiles=loom_guest_profiles_for_installation($installationId);
  ...
  if($action==='create'){
    $preferred=safe_token((string)($body['clientId']??''));if($preferred==='')throw new RuntimeException('Invalid client identity.');$policy=loom_continuity_guest_mode_policy($installationId,$preferred,$signals,true,true);if(!empty($policy['permanentRequired']))loom_identity_hub_permanent_required($policy);
