...
function loom_access_guest_catalog(): array {
  ...
  foreach(($s['profiles']??[]) as $pid=>$p){if(!is_array($p))continue;$pub=loom_guest_profile_public($p);$cid=safe_token((string)($pub['currentClientId']??''));$gp=$cid!==''?loom_access_profile_from_map('client',$cid,$profiles):null;$fallback=loom_clean_username((string)($pub['displayName']??''));$username=loom_clean_username((string)($gp['username']??''));$out[]=['type'=>'guest','id'=>(string)$pid,'label'=>$fallback!==''?$fallback:($username!==''?$username:'Guest'),'secondary'=>$cid,'currentClientId'=>$cid?:null,'generationStatus'=>$pub['generationStatus']??'active'];}
