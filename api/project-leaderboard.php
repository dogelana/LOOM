<?php
// @loom-file release=0.15.70 revision=3 policy=package-priority
...
$src=$method==='POST'?$body:$_GET;$clientId=safe_token((string)($src['clientId']??''));$project=safe_slug((string)($src['project']??''));$board=safe_token((string)($src['boardId']??''));$limit=max(1,min(100,(int)($src['limit']??10)));
if($clientId===''||$project===''||!project_dir($project)||$board==='')json_out(['ok'=>false,'error'=>'Invalid leaderboard request; clientId, project and boardId are required'],400);loom_capture_request_ip($clientId,$project);loom_enforce_project_access($project,$clientId);$owner=loom_project_state_owner($clientId);$module=loom_leaderboard_module_id($board);
