...
if($action==='register'){
  ...
  try{
    ...
    $identity=$project!==''?loom_project_identity_ensure($project,$clientId,true):null;
    $account=loom_account_public_status($clientId,$project);$continuityClaim=function_exists('loom_continuity_claim_for_user')?loom_continuity_claim_for_user($clientId,$uid,'register'):null;$generation=function_exists('loom_guest_profile_claim_generation')?loom_guest_profile_claim_generation($clientId,$uid,'register'):null;loom_audit_record('account.register',['clientId'=>$clientId,'userId'=>$uid,'project'=>$project,'details'=>['guestGeneration'=>$generation,'continuityClaim'=>$continuityClaim]],'Permanent LOOM account created and current guest payload claimed.');
    if(function_exists('loom_email_notify_user'))try{loom_email_notify_user('welcome',$user,['username'=>$username]);}catch(Throwable $e){}
    json_out([
      'ok'=>true,
      ...
      'attachedGuestHistories'=>$uid!==''?loom_guest_list_for_user($uid):[],'guestGeneration'=>$generation,'continuityClaim'=>$continuityClaim,
...
if($action==='login'){$email=trim((string)($body['email']??''));$password=(string)($body['password']??'');try{$user=loom_login_account($clientId,$email,$password);$uid=(string)($user['user_id']??$user['userId']??'');loom_reconcile_admin_identity($clientId);$identity=$project!==''?loom_project_identity_ensure($project,$clientId,true):null;$continuityClaim=function_exists('loom_continuity_claim_for_user')?loom_continuity_claim_for_user($clientId,$uid,'login'):null;$generation=function_exists('loom_guest_profile_claim_generation')?loom_guest_profile_claim_generation($clientId,$uid,'login'):null;$effective=loom_client_is_admin($clientId)?'Admin':'User';if(function_exists('loom_email_notify_user'))try{loom_email_notify_user('newSignIn',$user);}catch(Throwable $e){}loom_audit_record('account.login',['clientId'=>$clientId,'userId'=>$uid,'project'=>$project,'details'=>['guestGeneration'=>$generation,'continuityClaim'=>$continuityClaim]],'Permanent account signed in; eligible guest payload claimed automatically.');json_out(['ok'=>true,'message'=>'Signed in. Current guest payload was claimed automatically when eligible.','account'=>loom_account_public_status($clientId,$project),'guest'=>loom_guest_public(loom_guest_ensure_for_client($clientId)),'guestGeneration'=>$generation,'continuityClaim'=>$continuityClaim,'attachedGuestHistories'=>$uid!==''?loom_guest_list_for_user($uid):[],'projectIdentity'=>$identity,'privilege'=>['privilege'=>$effective,'isAdmin'=>$effective==='Admin','bootstrapped'=>false]]);}catch(Throwable $e){json_out(['ok'=>false,'error'=>$e->getMessage()],401);}}
