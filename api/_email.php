...
function loom_email_user_vars(array $u): array {$uid=(string)($u['user_id']??$u['userId']??'');$username='';if($uid!=='')try{$g=loom_global_profile_get('user',$uid);$username=function_exists('loom_global_profile_display_name')?loom_global_profile_display_name($g):(string)($g['username']??'');}catch(Throwable $e){}return ['userId'=>$uid,'username'=>$username];}
