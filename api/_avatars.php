...
function loom_avatar_migrate_legacy_once(string $project,string $clientId): void {
  ...
  $meta=['avatarMode'=>in_array($mode,['auto','global','project-default','custom'],true)?$mode:($mode==='loom-default'?'global':'auto')];
...
function loom_avatar_state(string $clientId,string $project): array {
  ...
  if($mode==='loom-default')$mode='global';if(!in_array($mode,['auto','global','project-default','custom'],true))$mode='auto';$custom=loom_avatar_find_file($project,$owner['type'],$owner['id']);if($mode==='custom'&&!$custom)$mode='auto';
...
function loom_avatar_set_mode(string $clientId,string $project,string $mode): array {
  if($mode==='loom-default')$mode='global';if(!in_array($mode,['auto','global','project-default','custom'],true))throw new RuntimeException('Invalid avatar mode.');loom_avatar_assert_mutation_access($clientId);$owner=loom_avatar_owner_for_client($clientId);$custom=loom_avatar_find_file($project,$owner['type'],$owner['id']);if($mode==='custom'&&!$custom)throw new RuntimeException('No custom avatar has been uploaded for this project identity yet.');loom_project_identity_set_avatar_meta($project,$owner['type'],$owner['id'],['avatarMode'=>$mode]);if(function_exists('loom_identity_audit'))loom_identity_audit('project-profile.avatar-mode.updated',['project'=>$project,'clientId'=>$clientId,'ownerType'=>$owner['type'],'ownerId'=>$owner['id'],'mode'=>$mode]);return loom_avatar_state($clientId,$project);
}
