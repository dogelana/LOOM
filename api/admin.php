<?php
// @loom-file release=0.15.64 revision=26 policy=package-priority
...
function settings_payload(string $project): array {
  ...
  $catalog=[];$frames=loom_html_framer_public_frames($project);$positionIds=array_map(fn($m)=>(string)$m['actionId'],$mods);foreach($frames as $f)$positionIds[]='html.frame.'.(string)$f['id'];$positionMap=loom_project_module_position_map($project,$positionIds);
  ...
  foreach($mods as $m){$id=$m['actionId'];$defaultHide=(bool)($m['manifestHideOnMobile']??false);$pos=$positionMap[$id]??['requested'=>null,'resolved'=>null,'source'=>'automatic','default'=>null,'explicit'=>null];$catalog[]=['actionId'=>$id,'name'=>$m['name'],'description'=>$m['description'],'scope'=>'project','source'=>$m['source']??'project','version'=>(string)($m['version']??'module'),'enabled'=>loom_project_module_enabled($project,$id,(bool)($m['manifestEnabled']??true)),'locked'=>false,'mobileVisibilitySupported'=>true,'hideOnMobile'=>loom_project_module_hide_on_mobile($project,$id,$defaultHide),'hideOnMobileDefault'=>$defaultHide,'chrome'=>loom_project_module_presentation_policy($project,$id,is_array($m['presentation']??null)?$m['presentation']:[]),'positionIndex'=>$pos['resolved'],'positionRequested'=>$pos['requested'],'positionSource'=>$pos['source'],'positionDefault'=>$pos['default'],'positionExplicit'=>$pos['explicit'],'positionSupported'=>true];}
  foreach($frames as $f){$id='html.frame.'.(string)$f['id'];$pos=$positionMap[$id]??['requested'=>null,'resolved'=>null,'source'=>'automatic','default'=>null,'explicit'=>null];$catalog[]=['actionId'=>$id,'name'=>(string)($f['title']??'HTML Frame'),'description'=>'HTML Framer package: '.(string)($f['zipName']??''),'scope'=>'project','source'=>'html-framer','version'=>'frame-r'.(string)($f['revision']??1),'enabled'=>loom_project_module_enabled($project,$id,(bool)($f['enabled']??true)),'locked'=>false,'mobileVisibilitySupported'=>true,'hideOnMobile'=>loom_project_module_hide_on_mobile($project,$id,false),'hideOnMobileDefault'=>false,'chrome'=>loom_project_module_presentation_policy($project,$id,['role'=>'content','collapsible'=>true]),'positionIndex'=>$pos['resolved'],'positionRequested'=>$pos['requested'],'positionSource'=>$pos['source'],'positionDefault'=>$pos['default'],'positionExplicit'=>$pos['explicit'],'positionSupported'=>true];}
...
$projectActionCaps=[
  ...
  'module-mobile-visibility'=>'project.modules','module-position-index'=>'project.modules','module-presentation-save'=>'project.modules','module-presentation-override'=>'project.modules','project-profile-save'=>'project.settings',
...
if($action==='module-position-index'){
  $toggleProject=safe_slug((string)($body['project']??''));$moduleId=safe_token((string)($body['moduleId']??''));if($toggleProject===''||!project_dir($toggleProject)||$moduleId==='')json_out(['ok'=>false,'error'=>'invalid-module-position'],400);$raw=$body['positionIndex']??null;$requested=($raw===null||$raw==='')?null:(is_numeric($raw)?(int)$raw:0);if($requested!==null&&$requested<1)json_out(['ok'=>false,'error'=>'Positioning Index Number must be 1 or greater, or blank.'],400);$known=[];foreach(scan_admin_modules($toggleProject) as $m)$known[]=(string)$m['actionId'];foreach(loom_html_framer_public_frames($toggleProject) as $f)$known[]='html.frame.'.(string)($f['id']??'');if(!in_array($moduleId,$known,true))json_out(['ok'=>false,'error'=>'project-module-not-found'],404);$resolution=loom_set_project_module_position_index($toggleProject,$moduleId,$requested,$known);json_out(['ok'=>true,'scope'=>'project','moduleId'=>$moduleId,'positionResolution'=>$resolution]+settings_payload($toggleProject));
}
