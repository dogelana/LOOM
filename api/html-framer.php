<?php
// @loom-file release=0.15.34 revision=8 policy=package-priority
...
if($action==='capture-url'){
  ...
  try{
    $settings=loom_read_admin_settings($project);$framerCfg=$settings['modules']['loom.html-framer']??[];$defaultHeight=max(240,min(1200,(int)($framerCfg['defaultFrameHeight']??520)));$defaultWidth=max(50,min(100,(int)($framerCfg['defaultFrameWidthPercent']??100)));
...
if($action==='analyze'||$action==='upload'){
  ...
  try{
    ...
    $defaultHeight=max(240,min(1200,(int)($framerCfg['defaultFrameHeight']??520)));$defaultWidth=max(50,min(100,(int)($framerCfg['defaultFrameWidthPercent']??100)));
