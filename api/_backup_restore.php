<?php
// @loom-file release=0.15.61 revision=14 policy=package-priority
...
function loom_backup_is_user_scoped_type(string $type): bool { return $type==='user'; }
function loom_backup_scope_label(string $type,?string $project=null): string {
  if($type==='full')return 'Entire LOOM installation';
  if($type==='user')return $project!==null&&trim($project)!==''?'User · '.trim($project):'Portable user';
...
function loom_backup_create(string $type,string $clientId,array $opts=[]): array {
  loom_backup_cleanup_old();loom_backup_require_zip();$clientId=safe_token($clientId);$requestedProject=safe_slug((string)($opts['project']??''));$project=loom_backup_is_project_scoped_type($type)?$requestedProject:'';$allProjects=false;$withData=false;$full=false;$userCapture=null;$scopeUserName=null;
  switch($type){case 'project':loom_backup_require_project($clientId,$project);break;case 'project-data':loom_backup_require_project($clientId,$project);$withData=true;break;case 'projects':loom_backup_require_global($clientId);$allProjects=true;break;case 'projects-data':loom_backup_require_global($clientId);$allProjects=true;$withData=true;break;case 'full':loom_backup_require_owner($clientId);$allProjects=true;$withData=true;$full=true;break;case 'user':loom_backup_require_owner($clientId);$uid=safe_token((string)($opts['userId']??''));if($uid==='')throw new RuntimeException('Choose a user to export.');$userCapture=loom_user_portability_capture($uid);$scopeUserName=(string)($userCapture['displayName']??$uid);break;default:throw new RuntimeException('Unknown export type.');}
  ...
  try{
    ...
    $scopeLabel=$type==='user'?loom_backup_scope_label('user',$scopeUserName):loom_backup_scope_label($type,$project);
    $meta=['backupId'=>$id,'type'=>$type,'project'=>loom_backup_is_project_scoped_type($type)?($project?:null):null,'userId'=>$type==='user'?($userCapture['account']['userId']??null):null,'scopeLabel'=>$scopeLabel,'createdAt'=>$manifest['createdAt'],'size'=>(int)filesize($zipPath),'sha256'=>loom_backup_sha256($zipPath),'file'=>basename($zipPath),'manifest'=>$manifest];@file_put_contents($dir.'/meta.json',json_encode($meta,JSON_UNESCAPED_SLASHES|JSON_PRETTY_PRINT),LOCK_EX);
    loom_audit_record('backup.export.created',['clientId'=>$clientId,'project'=>loom_backup_is_project_scoped_type($type)?($project?:null):null,'details'=>['backupId'=>$id,'type'=>$type,'scopeLabel'=>$scopeLabel,'projectCount'=>count($manifest['projects']),'userId'=>$meta['userId'],'databaseIncluded'=>$manifest['database']['included']]],'LOOM portable export created.');return $meta;
  }catch(Throwable $e){$zip->close();loom_backup_remove_tree($dir);throw $e;}
}
function loom_backup_list(string $clientId): array { loom_backup_cleanup_old();loom_backup_require_global($clientId);$out=[];$owner=loom_access_is_system_owner($clientId);foreach(glob(loom_backup_root().'/*/meta.json')?:[] as $f){$m=read_json_file($f);if(!is_array($m))continue;$type=(string)($m['type']??'');if(in_array($type,['full','user'],true)&&!$owner)continue;$changed=false;if(!loom_backup_is_project_scoped_type($type)&&array_key_exists('project',$m)&&$m['project']!==null){$m['project']=null;$changed=true;}$scope=$type==='user'?(string)($m['scopeLabel']??'Portable user'):loom_backup_scope_label($type,loom_backup_is_project_scoped_type($type)?(string)($m['project']??''):null);if((string)($m['scopeLabel']??'')!==$scope){$m['scopeLabel']=$scope;$changed=true;}if($changed)@file_put_contents($f,json_encode($m,JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE|JSON_PRETTY_PRINT),LOCK_EX);$m['downloadUrl']=web_base_path().'/api/backup.php?action=download&id='.rawurlencode((string)$m['backupId']).'&clientId='.rawurlencode($clientId);unset($m['manifest']);$out[]=$m;}usort($out,fn($a,$b)=>strcmp((string)($b['createdAt']??''),(string)($a['createdAt']??'')));return $out; }
...
function loom_backup_preview(string $clientId,array $staged): array {
  $m=$staged['manifest'];$type=(string)($m['exportType']??'');if(in_array($type,['full','user'],true))loom_backup_require_owner($clientId);else loom_backup_require_global($clientId);
...
function loom_backup_apply(string $clientId,array $staged,array $opts): array {
  $m=$staged['manifest'];$type=(string)($m['exportType']??'');$strategy=(string)($opts['strategy']??'create-new');if(!in_array($strategy,['create-new','replace','merge','skip'],true))throw new RuntimeException('Invalid import strategy.');if($type==='full'){loom_backup_require_owner($clientId);if((string)($opts['confirm']??'')!=='RESTORE LOOM')throw new RuntimeException('Type RESTORE LOOM to authorize a full-state restore.');}elseif($type==='user'){loom_backup_require_owner($clientId);if(!in_array($strategy,['create-new','merge'],true))throw new RuntimeException('Portable users support Create new or Merge only.');}else loom_backup_require_global($clientId);
