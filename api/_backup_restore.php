<?php
// @loom-file release=0.15.68 revision=15 policy=package-priority
...
function loom_backup_is_user_scoped_type(string $type): bool { return in_array($type,['user','guest'],true); }
function loom_backup_scope_label(string $type,?string $project=null): string {
  if($type==='full')return 'Entire LOOM installation';
  if($type==='user')return $project!==null&&trim($project)!==''?'User · '.trim($project):'Portable permanent user';
  if($type==='guest')return $project!==null&&trim($project)!==''?'Guest · '.trim($project):'Portable guest';
...
function loom_backup_copy_zip_prefix_filtered(ZipArchive $zip,string $prefix,string $dest,?callable $filter=null): int {
  $prefix=rtrim($prefix,'/').'/';$count=0;for($i=0;$i<$zip->numFiles;$i++){$name=(string)$zip->getNameIndex($i);if(!str_starts_with($name,$prefix)||str_ends_with($name,'/'))continue;$rel=substr($name,strlen($prefix));$rel=loom_backup_normalize_rel($rel);if($filter&&!$filter($rel))continue;$target=$dest.'/'.str_replace('/',DIRECTORY_SEPARATOR,$rel);ensure_dir(dirname($target));$in=$zip->getStream($name);if(!$in)throw new RuntimeException('Could not read '.$name);$out=@fopen($target,'wb');if(!$out){fclose($in);throw new RuntimeException('Could not write '.$target);}stream_copy_to_stream($in,$out);fclose($in);fclose($out);$count++;}return $count;
}
...
function loom_backup_create(string $type,string $clientId,array $opts=[]): array {
  loom_backup_cleanup_old();loom_backup_require_zip();$clientId=safe_token($clientId);$requestedProject=safe_slug((string)($opts['project']??''));$project=loom_backup_is_project_scoped_type($type)?$requestedProject:'';$allProjects=false;$withData=false;$full=false;$userCapture=null;$guestCapture=null;$scopePersonName=null;
  switch($type){case 'project':loom_backup_require_project($clientId,$project);break;case 'project-data':loom_backup_require_project($clientId,$project);$withData=true;break;case 'projects':loom_backup_require_global($clientId);$allProjects=true;break;case 'projects-data':loom_backup_require_global($clientId);$allProjects=true;$withData=true;break;case 'full':loom_backup_require_owner($clientId);$allProjects=true;$withData=true;$full=true;break;case 'user':loom_backup_require_owner($clientId);$uid=safe_token((string)($opts['userId']??''));if($uid==='')throw new RuntimeException('Choose a permanent user to export.');$userCapture=loom_user_portability_capture($uid);$scopePersonName=(string)($userCapture['displayName']??$uid);break;case 'guest':loom_backup_require_owner($clientId);$gid=safe_token((string)($opts['guestId']??''));if($gid==='')throw new RuntimeException('Choose a Guest identity to export.');$guestCapture=loom_guest_portability_capture($gid);$scopePersonName=(string)($guestCapture['displayName']??$gid);break;default:throw new RuntimeException('Unknown export type.');}
  ...
  try{
    if($type==='user'){
      $assetManifest=[];loom_user_portability_zip_add_assets($zip,$userCapture,$files,$assetManifest);$userCapture['assets']=$assetManifest;
      $payload=json_encode($userCapture,JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE|JSON_PRETTY_PRINT);loom_backup_add_bytes($zip,$payload?:'{}','payload/user/user-portable.json',$files);
      $graphCounts=[];foreach((array)($userCapture['graph']??[]) as $k=>$v)if(is_array($v))$graphCounts[$k]=count($v);
      $manifest['user']=['userId'=>$userCapture['account']['userId']??null,'displayName'=>$userCapture['displayName']??null,'email'=>$userCapture['account']['email']??null,'privilege'=>$userCapture['account']['privilege']??'User','wasSystemOwner'=>(bool)($userCapture['wasSystemOwner']??false),'projectRefs'=>$userCapture['projectRefs']??[],'graphCounts'=>$graphCounts,'assetCount'=>count($assetManifest),'payloadPath'=>'payload/user/user-portable.json'];
      $manifest['dataClasses']['included']=['permanent-account','password-hash','global-profile','project-identities','project-user-state','linked-browser-identities','attached-guest-history','activity-and-audit-history','permissions-and-moderation','continuity-and-referrals','user-media','database-user-rows'];
      $manifest['dataClasses']['excluded']=['active-auth-sessions','password-reset-tokens','guest-recovery-secrets','database-credentials','server-secrets','system-owner-binding','unrelated-users'];
      $manifest['database']['included']=is_array($userCapture['database']??null);$manifest['database']['authenticationHashesIncluded']=true;$manifest['database']['tableCount']=is_array($userCapture['database']['tables']??null)?count($userCapture['database']['tables']):0;
      $manifest['security']['passwordHashIncluded']=true;$manifest['security']['authSessionsIncluded']=false;$manifest['security']['systemOwnerBindingIncluded']=false;$manifest['compatibility']['restoresMissingProjectReferencesAsDormantData']=true;
    }elseif($type==='guest'){
      $assetManifest=[];loom_guest_portability_zip_add_assets($zip,$guestCapture,$files,$assetManifest);$guestCapture['assets']=$assetManifest;
      $payload=json_encode($guestCapture,JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE|JSON_PRETTY_PRINT);loom_backup_add_bytes($zip,$payload?:'{}','payload/guest/guest-portable.json',$files);
      $graphCounts=[];foreach((array)($guestCapture['graph']??[]) as $k=>$v)if(is_array($v))$graphCounts[$k]=count($v);
      $manifest['guest']=['guestId'=>$guestCapture['guest']['guestId']??null,'displayName'=>$guestCapture['displayName']??null,'wasSystemOwner'=>(bool)($guestCapture['wasSystemOwner']??false),'projectRefs'=>$guestCapture['projectRefs']??[],'graphCounts'=>$graphCounts,'assetCount'=>count($assetManifest),'payloadPath'=>'payload/guest/guest-portable.json'];
      $manifest['dataClasses']['included']=['standalone-guest-identity','linked-browser-identities','guest-profile-lineage','global-client-profile','project-identities','project-user-state','activity-and-audit-history','permissions-and-moderation','continuity-and-referrals','guest-media','database-guest-rows'];
      $manifest['dataClasses']['excluded']=['permanent-account','active-auth-sessions','guest-recovery-secrets','database-credentials','server-secrets','system-owner-binding','unrelated-users-and-guests'];
      $manifest['database']['included']=is_array($guestCapture['database']??null);$manifest['database']['authenticationHashesIncluded']=false;$manifest['database']['tableCount']=is_array($guestCapture['database']['tables']??null)?count($guestCapture['database']['tables']):0;
      $manifest['security']['guestRecoverySecretsIncluded']=false;$manifest['security']['authSessionsIncluded']=false;$manifest['security']['systemOwnerBindingIncluded']=false;$manifest['compatibility']['restoresMissingProjectReferencesAsDormantData']=true;
    }else{
    ...
    $scopeLabel=in_array($type,['user','guest'],true)?loom_backup_scope_label($type,$scopePersonName):loom_backup_scope_label($type,$project);
    $meta=['backupId'=>$id,'type'=>$type,'project'=>loom_backup_is_project_scoped_type($type)?($project?:null):null,'userId'=>$type==='user'?($userCapture['account']['userId']??null):null,'guestId'=>$type==='guest'?($guestCapture['guest']['guestId']??null):null,'scopeLabel'=>$scopeLabel,'createdAt'=>$manifest['createdAt'],'size'=>(int)filesize($zipPath),'sha256'=>loom_backup_sha256($zipPath),'file'=>basename($zipPath),'manifest'=>$manifest];@file_put_contents($dir.'/meta.json',json_encode($meta,JSON_UNESCAPED_SLASHES|JSON_PRETTY_PRINT),LOCK_EX);
    loom_audit_record('backup.export.created',['clientId'=>$clientId,'project'=>loom_backup_is_project_scoped_type($type)?($project?:null):null,'details'=>['backupId'=>$id,'type'=>$type,'scopeLabel'=>$scopeLabel,'projectCount'=>count($manifest['projects']),'userId'=>$meta['userId'],'guestId'=>$meta['guestId'],'databaseIncluded'=>$manifest['database']['included']]],'LOOM portable export created.');return $meta;
  }catch(Throwable $e){$zip->close();loom_backup_remove_tree($dir);throw $e;}
}
function loom_backup_list(string $clientId): array { loom_backup_cleanup_old();loom_backup_require_global($clientId);$out=[];$owner=loom_access_is_system_owner($clientId);foreach(glob(loom_backup_root().'/*/meta.json')?:[] as $f){$m=read_json_file($f);if(!is_array($m))continue;$type=(string)($m['type']??'');if(in_array($type,['full','user','guest'],true)&&!$owner)continue;$changed=false;if(!loom_backup_is_project_scoped_type($type)&&array_key_exists('project',$m)&&$m['project']!==null){$m['project']=null;$changed=true;}$scope=in_array($type,['user','guest'],true)?(string)($m['scopeLabel']??loom_backup_scope_label($type)):loom_backup_scope_label($type,loom_backup_is_project_scoped_type($type)?(string)($m['project']??''):null);if((string)($m['scopeLabel']??'')!==$scope){$m['scopeLabel']=$scope;$changed=true;}if($changed)@file_put_contents($f,json_encode($m,JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE|JSON_PRETTY_PRINT),LOCK_EX);$m['downloadUrl']=web_base_path().'/api/backup.php?action=download&id='.rawurlencode((string)$m['backupId']).'&clientId='.rawurlencode($clientId);unset($m['manifest']);$out[]=$m;}usort($out,fn($a,$b)=>strcmp((string)($b['createdAt']??''),(string)($a['createdAt']??'')));return $out; }
...
function loom_backup_project_import_capabilities(ZipArchive $z,array $p): array {
  $source=safe_slug((string)($p['slug']??''));$runtimePrefix=(string)($p['runtimePath']??('payload/projects/'.$source.'/project'));$overlayPrefix=(string)($p['overlayPath']??('payload/projects/'.$source.'/overlay'));$htmlPrefix=(string)($p['htmlFramerPath']??('payload/projects/'.$source.'/html-framer'));$dataPrefix=(string)($p['dataPath']??('payload/projects/'.$source.'/instance-data'));$statePath=(string)($p['projectStatePath']??('payload/projects/'.$source.'/project-state.json'));
  $hasStructure=loom_backup_zip_has_prefix($z,$runtimePrefix)||loom_backup_zip_has_prefix($z,$overlayPrefix)||loom_backup_zip_has_prefix($z,$htmlPrefix)||$z->locateName('payload/projects/'.$source.'/project-overrides.json')!==false||$z->locateName('payload/projects/'.$source.'/admin-settings.json')!==false;
  $hasInstanceData=loom_backup_zip_has_prefix($z,$dataPrefix);$hasProjectState=$z->locateName($statePath)!==false;$hasData=$hasInstanceData||$hasProjectState;$modes=[];if($hasStructure)$modes[]='project';if($hasData)$modes[]='data';if($hasStructure&&$hasData)$modes[]='both';$recommended=$hasStructure&&$hasData?'both':($hasStructure?'project':($hasData?'data':'project'));
  return ['hasProjectStructure'=>$hasStructure,'hasProjectData'=>$hasData,'hasInstanceData'=>$hasInstanceData,'hasProjectState'=>$hasProjectState,'availableImportModes'=>$modes,'recommendedImportMode'=>$recommended];
}
function loom_backup_preview(string $clientId,array $staged): array {
  $m=$staged['manifest'];$type=(string)($m['exportType']??'');if(in_array($type,['full','user','guest'],true))loom_backup_require_owner($clientId);else loom_backup_require_global($clientId);
  if($type==='user'){$z=new ZipArchive();if($z->open($staged['path'])!==true)throw new RuntimeException('Could not open staged user import for preview.');try{$path=(string)($m['user']['payloadPath']??'payload/user/user-portable.json');$raw=$z->getFromName($path);$payload=is_string($raw)?json_decode($raw,true):null;if(!is_array($payload))throw new RuntimeException('Portable user payload is missing or invalid.');$user=loom_user_portability_preview_payload($payload);}finally{$z->close();}return ['importId'=>$staged['importId'],'exportType'=>'user','sourceLoomVersion'=>$m['sourceLoomVersion']??null,'createdAt'=>$m['createdAt']??null,'projects'=>[],'database'=>$m['database']??[],'dataClasses'=>$m['dataClasses']??[],'verification'=>$staged['verify'],'requiresSystemOwner'=>true,'strategies'=>['create-new','merge'],'availableImportModes'=>[],'recommendedImportMode'=>null,'user'=>$user];}
  if($type==='guest'){$z=new ZipArchive();if($z->open($staged['path'])!==true)throw new RuntimeException('Could not open staged Guest import for preview.');try{$path=(string)($m['guest']['payloadPath']??'payload/guest/guest-portable.json');$raw=$z->getFromName($path);$payload=is_string($raw)?json_decode($raw,true):null;if(!is_array($payload))throw new RuntimeException('Portable Guest payload is missing or invalid.');$guest=loom_guest_portability_preview_payload($payload);}finally{$z->close();}return ['importId'=>$staged['importId'],'exportType'=>'guest','sourceLoomVersion'=>$m['sourceLoomVersion']??null,'createdAt'=>$m['createdAt']??null,'projects'=>[],'database'=>$m['database']??[],'dataClasses'=>$m['dataClasses']??[],'verification'=>$staged['verify'],'requiresSystemOwner'=>true,'strategies'=>['create-new','merge'],'availableImportModes'=>[],'recommendedImportMode'=>null,'guest'=>$guest];}
  $projects=[];$z=new ZipArchive();if($z->open($staged['path'])!==true)throw new RuntimeException('Could not open staged import for preview.');try{foreach(($m['projects']??[]) as $p){$slug=safe_slug((string)($p['slug']??''));$cap=loom_backup_project_import_capabilities($z,$p);$projects[]=['slug'=>$slug,'name'=>$p['name']??humanize_project_slug($slug),'exists'=>project_dir($slug)!==null,'currentSource'=>loom_project_source($slug),'willImportAs'=>'instance','htmlFramerIncluded'=>(bool)($p['htmlFramerIncluded']??false),'htmlFramerFrameCount'=>(int)($p['htmlFramerFrameCount']??0)]+$cap;}}finally{$z->close();}
  $hasStructure=(bool)array_filter($projects,fn($x)=>!empty($x['hasProjectStructure']));$hasData=(bool)array_filter($projects,fn($x)=>!empty($x['hasProjectData']));$bundleModes=[];if($hasStructure)$bundleModes[]='project';if($hasData)$bundleModes[]='data';if($hasStructure&&$hasData)$bundleModes[]='both';
  return ['importId'=>$staged['importId'],'exportType'=>$type,'sourceLoomVersion'=>$m['sourceLoomVersion']??null,'createdAt'=>$m['createdAt']??null,'projects'=>$projects,'database'=>$m['database']??[],'dataClasses'=>$m['dataClasses']??[],'verification'=>$staged['verify'],'requiresSystemOwner'=>$type==='full','strategies'=>['create-new','replace','merge','skip'],'availableImportModes'=>$bundleModes,'recommendedImportMode'=>$hasStructure&&$hasData?'both':($hasStructure?'project':($hasData?'data':'project'))];
}
...
function loom_backup_zip_prefix_top_entries(ZipArchive $z,string $prefix): array {
  $prefix=rtrim(str_replace('\\','/',$prefix),'/').'/';$out=[];for($i=0;$i<$z->numFiles;$i++){$name=(string)$z->getNameIndex($i);if(!str_starts_with($name,$prefix))continue;$rest=substr($name,strlen($prefix));if($rest===''||str_ends_with($rest,'/'))continue;$first=explode('/',$rest,2)[0]??'';if($first!=='')$out[$first]=true;}return array_keys($out);
}
function loom_backup_merge_project_state_json(string $incoming,string $targetPath): void {
  $src=json_decode($incoming,true);if(!is_array($src))throw new RuntimeException('Project state payload is invalid.');$dst=read_json_file($targetPath)?:[];if(!is_array($dst))$dst=[];foreach($src as $key=>$row)$dst[$key]=$row;ensure_dir(dirname($targetPath));@file_put_contents($targetPath,json_encode($dst,JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE|JSON_PRETTY_PRINT),LOCK_EX);
}
function loom_backup_restore_project_from_zip(ZipArchive $z,array $p,string $target,string $strategy,string $importMode='auto'): array {
  $source=safe_slug((string)($p['slug']??''));$target=safe_slug($target?:$source);if($source===''||$target==='')throw new RuntimeException('Invalid project mapping.');$cap=loom_backup_project_import_capabilities($z,$p);$allowed=$cap['availableImportModes'];if($importMode==='auto'||$importMode==='')$importMode=(string)$cap['recommendedImportMode'];if(!in_array($importMode,['project','data','both'],true)||!in_array($importMode,$allowed,true))throw new RuntimeException('The '.$source.' bundle does not contain the requested '.($importMode==='data'?'project data':($importMode==='project'?'project files':'project files + data')).'.');$includeStructure=in_array($importMode,['project','both'],true);$includeData=in_array($importMode,['data','both'],true);
  $exists=project_dir($target)!==null;if($strategy==='skip'&&$exists)return ['source'=>$source,'target'=>$target,'status'=>'skipped','importMode'=>$importMode];if($strategy==='create-new'&&$exists)throw new RuntimeException('Project '.$target.' already exists. Choose Replace, Merge, or a new slug.');if($includeData&&!$includeStructure&&!$exists)throw new RuntimeException('Data-only import requires an existing destination project. Import project files first or choose Project + data.');
  $storage=loom_instance_project_storage_path($target);$runtime=$storage.'/project';$rollback=null;if($exists&&$strategy==='replace'){$rollback=loom_backup_snapshot_path('project-'.$target);if(is_dir($storage))loom_backup_copy_tree($storage,$rollback.'/instance-project');else{$cur=project_dir($target);if($cur)loom_backup_copy_tree($cur,$rollback.'/release-project');}$statePath=loom_project_state_file($target);if(is_file($statePath)){ensure_dir($rollback.'/project-state');@copy($statePath,$rollback.'/project-state/'.$target.'.json');}$settingsPath=loom_admin_settings_file($target);if(is_file($settingsPath)){ensure_dir($rollback.'/admin-settings');@copy($settingsPath,$rollback.'/admin-settings/'.$target.'.json');}}
  ensure_dir($storage);
  $htmlPrefix=(string)($p['htmlFramerPath']??('payload/projects/'.$source.'/html-framer'));$dataPrefix=(string)($p['dataPath']??('payload/projects/'.$source.'/instance-data'));$declaresHtmlFramer=array_key_exists('htmlFramerIncluded',$p);$hasHtmlFramer=loom_backup_zip_has_prefix($z,$htmlPrefix);$declaredFrameCount=(int)($p['htmlFramerFrameCount']??0);if($includeStructure&&$declaresHtmlFramer&&($p['htmlFramerIncluded']??false)&&$declaredFrameCount>0&&!$hasHtmlFramer)throw new RuntimeException('Portable bundle declares HTML Framer packages but their payload is missing.');$legacyDataHasHtmlFramer=loom_backup_zip_has_prefix($z,$dataPrefix.'/html-framer');
  if($includeStructure){
    if($exists&&$strategy==='replace'){loom_backup_remove_tree($runtime);loom_backup_remove_tree($storage.'/overlay');@unlink($storage.'/project-overrides.json');if($declaresHtmlFramer||$legacyDataHasHtmlFramer)loom_backup_remove_tree($storage.'/html-framer');}
    loom_backup_copy_zip_prefix($z,'payload/projects/'.$source.'/project',$runtime);loom_backup_copy_zip_prefix($z,'payload/projects/'.$source.'/overlay',$storage.'/overlay');$projectDefault=$runtime.'/project.default.json';if(is_file($projectDefault)&&$target!==$source){$pd=read_json_file($projectDefault);if(is_array($pd)){$pd['slug']=$target;@file_put_contents($projectDefault,json_encode($pd,JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE|JSON_PRETTY_PRINT)."\n",LOCK_EX);}}
    ...
    elseif($legacyDataHasHtmlFramer)loom_backup_copy_zip_prefix($z,$dataPrefix.'/html-framer',$storage.'/html-framer');
  }
  if($includeData){
    // Replace only paths actually present in the incoming data payload. Unrelated project
    // storage is left alone, which makes overwrite imports intentionally gentle.
    if($exists&&$strategy==='replace')foreach(loom_backup_zip_prefix_top_entries($z,$dataPrefix) as $first){if(in_array($first,['project','overlay','project-overrides.json','html-framer'],true))continue;loom_backup_remove_tree($storage.'/'.$first);}
    loom_backup_copy_zip_prefix_filtered($z,$dataPrefix,$storage,function(string $rel): bool { return !str_starts_with(str_replace('\\','/',$rel),'html-framer/'); });$projectStatePath=(string)($p['projectStatePath']??('payload/projects/'.$source.'/project-state.json'));$projectState=$z->getFromName($projectStatePath);if(is_string($projectState)){$statePath=loom_project_state_file($target);if($strategy==='merge')loom_backup_merge_project_state_json($projectState,$statePath);else{ensure_dir(dirname($statePath));@file_put_contents($statePath,$projectState,LOCK_EX);}}
  }
  return ['source'=>$source,'target'=>$target,'status'=>'imported','strategy'=>$strategy,'importMode'=>$importMode,'projectFilesRestored'=>$includeStructure,'projectDataRestored'=>$includeData,'rollback'=>$rollback,'htmlFramerRestored'=>$includeStructure&&($hasHtmlFramer||$legacyDataHasHtmlFramer)];
}
...
function loom_backup_apply(string $clientId,array $staged,array $opts): array {
  $m=$staged['manifest'];$type=(string)($m['exportType']??'');$strategy=(string)($opts['strategy']??'create-new');if(!in_array($strategy,['create-new','replace','merge','skip'],true))throw new RuntimeException('Invalid import strategy.');if($type==='full'){loom_backup_require_owner($clientId);if((string)($opts['confirm']??'')!=='RESTORE LOOM')throw new RuntimeException('Type RESTORE LOOM to authorize a full-state restore.');}elseif(in_array($type,['user','guest'],true)){loom_backup_require_owner($clientId);if(!in_array($strategy,['create-new','merge'],true))throw new RuntimeException('Portable people support Create new or Merge only.');}else loom_backup_require_global($clientId);
  $z=new ZipArchive();if($z->open($staged['path'])!==true)throw new RuntimeException('Could not open staged import.');
  if($type==='user'){try{$path=(string)($m['user']['payloadPath']??'payload/user/user-portable.json');$raw=$z->getFromName($path);$payload=is_string($raw)?json_decode($raw,true):null;if(!is_array($payload))throw new RuntimeException('Portable user payload is missing or invalid.');$assets=is_array($payload['assets']??null)?$payload['assets']:[];$userResult=loom_user_portability_import($z,$payload,$assets,$strategy);}finally{$z->close();}loom_audit_record('backup.import.applied',['clientId'=>$clientId,'details'=>['importId'=>$staged['importId'],'exportType'=>'user','strategy'=>$strategy,'user'=>$userResult]],'Portable LOOM user import applied.');return ['user'=>$userResult];}
  if($type==='guest'){try{$path=(string)($m['guest']['payloadPath']??'payload/guest/guest-portable.json');$raw=$z->getFromName($path);$payload=is_string($raw)?json_decode($raw,true):null;if(!is_array($payload))throw new RuntimeException('Portable Guest payload is missing or invalid.');$assets=is_array($payload['assets']??null)?$payload['assets']:[];$guestResult=loom_guest_portability_import($z,$payload,$assets,$strategy);}finally{$z->close();}loom_audit_record('backup.import.applied',['clientId'=>$clientId,'details'=>['importId'=>$staged['importId'],'exportType'=>'guest','strategy'=>$strategy,'guest'=>$guestResult]],'Portable LOOM Guest import applied.');return ['guest'=>$guestResult];}
  $result=['projects'=>[],'database'=>null,'globalState'=>null];try{foreach(($m['projects']??[]) as $p){$source=safe_slug((string)($p['slug']??''));$map=is_array($opts['projectMap']??null)?($opts['projectMap'][$source]??$source):$source;$projStrategy=is_array($opts['projectStrategies']??null)?($opts['projectStrategies'][$source]??$strategy):$strategy;$projMode=is_array($opts['projectImportModes']??null)?($opts['projectImportModes'][$source]??($opts['importMode']??'auto')):($opts['importMode']??'auto');$result['projects'][]=loom_backup_restore_project_from_zip($z,$p,safe_slug((string)$map),(string)$projStrategy,(string)$projMode);}
    if($type==='full'){
  ...
  loom_audit_record('backup.import.applied',['clientId'=>$clientId,'details'=>['importId'=>$staged['importId'],'exportType'=>$type,'strategy'=>$strategy,'importMode'=>$opts['importMode']??'auto','projects'=>$result['projects'],'database'=>$result['database']]],'LOOM portable import applied.');return $result;
}
...
require_once __DIR__.'/_user_portability.php';
require_once __DIR__.'/_guest_portability.php';
