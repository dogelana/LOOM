<?php
// @loom-file release=0.15.75 revision=71 policy=package-priority
...
function loom_deployment_gate_status(): ?array {
  ...
  $raw=@file_get_contents($marker);$data=is_string($raw)?json_decode($raw,true):null;
  // The gate is a leased transient file, never durable installation state. A
  // malformed marker cannot be allowed to brick every PHP endpoint forever.
  if(!is_array($data)||!$data){@unlink($marker);return null;}
  $now=microtime(true);$expires=(float)($data['expires_at_epoch']??0);$schema=(string)($data['schema']??'');
  if($expires>0){if($expires<=$now){@unlink($marker);return null;}return $data;}
  // Bridge 8.3+ v2 markers always carry expires_at_epoch. Missing lease data on
  // a v2 marker is therefore corrupt/stale and must fail open immediately.
  if($schema==='loom-deployment-gate/v2'){@unlink($marker);return null;}
  // Very old pre-lease markers are honored only for a short compatibility grace
  // window based on their last write time. They can never become permanent locks.
  $updated=(float)($data['updated_epoch']??0);$mtime=(float)(@filemtime($marker)?:0);$fresh=max($updated,$mtime);
  if($fresh<=0||($now-$fresh)>180){@unlink($marker);return null;}
  return $data;
}
...
function loom_release_version(string $fallback='0.15.67'): string { $m=read_json_file(root_dir().'/.loom-deployment.json'); $v=trim((string)($m['loom_release']??'')); return $v!==''?$v:$fallback; }
...
function loom_project_asset_url(string $project,string $relative): ?string {
  ...
  $url=web_base_path().'/api/project-asset.php?project='.rawurlencode($slug).'&path='.rawurlencode($relative).'&v='.rawurlencode($v);
  if(function_exists('loom_access_project_is_private')&&loom_access_project_is_private($slug)&&function_exists('loom_access_project_asset_signature'))$url.='&ps='.rawurlencode(loom_access_project_asset_signature($slug,$relative));
  return $url;
}
