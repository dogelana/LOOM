<?php
// @loom-file release=0.15.66 revision=64 policy=package-priority
...
function loom_deployment_gate_status(): ?array {
  ...
  if($expires>0&&$expires<=microtime(true)){@unlink($marker);return null;}
  return $data;
}
...
function loom_release_version(string $fallback='0.15.66'): string { $m=read_json_file(root_dir().'/.loom-deployment.json'); $v=trim((string)($m['loom_release']??'')); return $v!==''?$v:$fallback; }
